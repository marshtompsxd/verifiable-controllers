        ...
        ),
{
    ...
    let input: (Option<Message<EmptyAPI>>, Option<ObjectRef>) = (None, Some(rabbitmq.object_ref()));
...
proof fn lemma_receives_some_resp_at_rabbitmq_step_with_rabbitmq(
    spec: TempPred<RMQCluster>, rabbitmq: RabbitmqClusterView, req_msg: Message<EmptyAPI>, step: RabbitmqReconcileStep
)
    requires
        spec.entails(always(lift_action(RMQCluster::next()))),
        spec.entails(tla_forall(|i| RMQCluster::kubernetes_api_next().weak_fairness(i))),
...
proof fn lemma_from_resp_in_flight_at_some_step_to_pending_req_in_flight_at_next_step(
    spec: TempPred<RMQCluster>, rabbitmq: RabbitmqClusterView, resp_msg: Message<EmptyAPI>, step: RabbitmqReconcileStep, result_step: RabbitmqReconcileStep
)
    requires
        spec.entails(always(lift_action(RMQCluster::next()))),
        spec.entails(tla_forall(|i| RMQCluster::controller_next().weak_fairness(i))),
...
proof fn lemma_receives_ok_resp_at_after_get_stateful_set_step_with_rabbitmq(
    spec: TempPred<RMQCluster>, rabbitmq: RabbitmqClusterView, req_msg: Message<EmptyAPI>, object: DynamicObjectView
)
    requires
        spec.entails(always(lift_action(RMQCluster::next()))),
        spec.entails(tla_forall(|i| RMQCluster::kubernetes_api_next().weak_fairness(i))),
...
proof fn lemma_from_after_get_stateful_set_step_to_after_update_stateful_set_step(
    spec: TempPred<RMQCluster>, rabbitmq: RabbitmqClusterView, resp_msg: Message<EmptyAPI>, object: DynamicObjectView
)
    requires
        spec.entails(always(lift_action(RMQCluster::next()))),
        spec.entails(tla_forall(|i| RMQCluster::controller_next().weak_fairness(i))),
...
proof fn lemma_receives_not_found_resp_at_after_get_server_config_map_step_with_rabbitmq(
    spec: TempPred<RMQCluster>, rabbitmq: RabbitmqClusterView, req_msg: Message<EmptyAPI>
)
    requires
        spec.entails(always(lift_action(RMQCluster::next()))),
        spec.entails(tla_forall(|i| RMQCluster::kubernetes_api_next().weak_fairness(i))),
...
proof fn lemma_from_after_get_server_config_map_step_to_after_create_server_config_map_step(
    spec: TempPred<RMQCluster>, rabbitmq: RabbitmqClusterView, resp_msg: Message<EmptyAPI>
)
    requires
        spec.entails(always(lift_action(RMQCluster::next()))),
        spec.entails(tla_forall(|i| RMQCluster::controller_next().weak_fairness(i))),
