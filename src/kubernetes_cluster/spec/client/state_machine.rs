...
pub open spec fn create_custom_resource() -> ClientAction<K, E> {
    Action {
...
pub open spec fn delete_custom_resource() -> ClientAction<K, E> {
    Action {
        precondition: |input: ClientActionInput<K>, s: ClientState| {
            &&& input.cr.metadata().name.is_Some()
            &&& input.cr.metadata().namespace.is_Some()
...
pub open spec fn update_custom_resource() -> ClientAction<K, E> {
    Action {
        precondition: |input: ClientActionInput<K>, s: ClientState| {
            &&& input.cr.metadata().name.is_Some()
            &&& input.cr.metadata().namespace.is_Some()
...
pub open spec fn client() -> ClientStateMachine<K, E> {
    StateMachine {
        init: |s: ClientState| {
            true
        },
