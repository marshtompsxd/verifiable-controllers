...
pub proof fn kubernetes_api_action_pre_implies_next_pre(
    action: KubernetesAPIAction<E>, input: Option<Message<E>>
)
    requires
...
pub proof fn exists_next_kubernetes_api_step(
    action: KubernetesAPIAction<E>, input: KubernetesAPIActionInput<E>, s: KubernetesAPIState
)
    requires
        Self::kubernetes_api().actions.contains(action),
...
pub proof fn controller_action_pre_implies_next_pre(
    action: ControllerAction<K, E, R>, input: (Option<Message<E>>, Option<ObjectRef>)
)
    requires
...
pub proof fn builtin_controllers_action_pre_implies_next_pre(
    action: BuiltinControllersAction<E>, input: (BuiltinControllerChoice, ObjectRef)
)
    requires
...
pub proof fn exists_next_builtin_controllers_step(
    action: BuiltinControllersAction<E>, input: BuiltinControllersActionInput, s: BuiltinControllersState
)
    requires
        Self::builtin_controllers().actions.contains(action),
        (action.precondition)(input, s),
