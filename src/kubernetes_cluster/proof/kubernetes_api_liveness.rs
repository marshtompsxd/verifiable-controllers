...
pub proof fn lemma_pre_leads_to_post_by_kubernetes_api(
    spec: TempPred<Self>, input: Option<Message<E>>, next: ActionPred<Self>, action: KubernetesAPIAction<E>,
...
{
    use_tla_forall::<Self, Option<Message<E>>>(spec, |i| Self::kubernetes_api_next().weak_fairness(i), input);
...
pub proof fn lemma_pre_leads_to_post_by_builtin_controllers(
    spec: TempPred<Self>, input: (BuiltinControllerChoice, ObjectRef), next: ActionPred<Self>, action: BuiltinControllersAction<E>,
    pre: StatePred<Self>, post: StatePred<Self>
)
    requires
...
(spec: TempPred<Self>, msg: Message<E>, key: ObjectRef)
    requires
        ...
        ),
{
    ...
    let post = |s: Self| exists |resp_msg: Message<E>| {
        &&& #[trigger] s.message_in_flight(resp_msg)
        &&& Message::resp_msg_matches_req_msg(resp_msg, msg)
    };
...
pub proof fn lemma_create_req_leads_to_res_exists(spec: TempPred<Self>, msg: Message<E>)
    requires
        spec.entails(always(lift_state(Self::busy_disabled()))),
        spec.entails(always(lift_action(Self::next()))),
        spec.entails(tla_forall(|i| Self::kubernetes_api_next().weak_fairness(i))),
...
/// proof strategy.
pub open spec fn every_new_req_msg_if_in_flight_then_satisfies(requirements: FnSpec(Message<E>, Self) -> bool) -> ActionPred<Self> {
    |s: Self, s_prime: Self| {
        forall |msg: Message<E>|
...
pub open spec fn every_in_flight_req_msg_satisfies(requirements: FnSpec(Message<E>, Self) -> bool) -> StatePred<Self> {
    |s: Self| {
        forall |msg: Message<E>|
            #[trigger] s.message_in_flight(msg)
            && msg.dst.is_KubernetesAPI()
...
pub proof fn lemma_true_leads_to_always_every_in_flight_req_msg_satisfies(
    spec: TempPred<Self>, requirements: FnSpec(Message<E>, Self) -> bool
)
    requires
...
pub proof fn lemma_some_rest_id_leads_to_always_every_in_flight_req_msg_satisfies_with_rest_id(
    spec: TempPred<Self>, requirements: FnSpec(Message<E>, Self) -> bool, rest_id: nat
)
    requires
        spec.entails(always(lift_action(Self::every_new_req_msg_if_in_flight_then_satisfies(requirements)))),
        spec.entails(always(lift_state(Self::every_in_flight_msg_has_lower_id_than_allocator()))),
    ...
    if msg_num == 0 {
        ...
        assert_by(pending_requests_num_is_zero == no_more_pending_requests, {
            ...
            implies pending_requests_num_is_zero.satisfied_by(ex) by {
                ...
                len_is_zero_means_count_for_each_value_is_zero::<Message<E>>(
        ...
        let no_more_pending_requests = lift_state(Self::no_req_before_rest_id_is_in_flight(rest_id));
        let pending_req_in_flight = |msg: Message<E>| lift_state(|s: Self| {
            s.network_state.in_flight.filter(api_request_msg_before(rest_id)).count(msg) > 0
        });
        let pending_requests_num_is_msg_num_and_pending_req_in_flight = |msg: Message<E>| lift_state(|s: Self| {
            &&& s.network_state.in_flight.filter(api_request_msg_before(rest_id)).len() == msg_num
            &&& s.network_state.in_flight.filter(api_request_msg_before(rest_id)).count(msg) > 0
        });
        ...
        // to reason about.
        assert forall |msg: Message<E>| spec.entails(
...
proof fn pending_requests_num_decreases(
    spec: TempPred<Self>, rest_id: RestId, msg_num: nat, msg: Message<E>
)
    requires
        msg_num > 0,
        spec.entails(always(lift_state(Self::busy_disabled()))),
