...
pub open spec fn resp_matches_at_most_one_pending_req(
    resp_msg: Message<E>, cr_key: ObjectRef
) -> StatePred<Self> {
    |s: Self| {
        ...
        ==> (
            forall |other_key: ObjectRef|
                ...
                && other_key != cr_key
                ==> !Message::resp_msg_matches_req_msg(resp_msg, s.reconcile_state_of(other_key).pending_req_msg.get_Some_0())
            )
    }
}
...
pub open spec fn resp_if_matches_pending_req_then_no_other_resp_matches(
    ...
    |s: Self| {
        ...
        && Message::resp_msg_matches_req_msg(resp_msg, s.reconcile_state_of(cr_key).pending_req_msg.get_Some_0())
        ==> (
            forall |other_resp: Message<E>| other_resp != resp_msg && #[trigger] s.message_in_flight(other_resp)
...
pub proof fn lemma_forall_always_resp_if_matches_pending_req_then_no_other_resp_matches(
    ...
    ensures
        Self::sm_spec().entails(
            tla_forall(|resp_msg: Message<E>| always(lift_state(Self::resp_if_matches_pending_req_then_no_other_resp_matches(resp_msg, cr_key))))
        ),
{
        ...
        ),
{
    ...
    let a_to_p = |resp_msg: Message<E>| lift_state(Self::resp_if_matches_pending_req_then_no_other_resp_matches(resp_msg, cr_key));
    let a_to_always_p = |resp_msg: Message<E>| always(a_to_p(resp_msg));
    assert forall |resp_msg: Message<E>| spec.entails(#[trigger] a_to_always_p(resp_msg))
    by {
    ...
    assert forall |ex| #[trigger] tla_forall(a_to_p).satisfied_by(ex) implies forall_a_to_p.satisfied_by(ex) by {
        assert forall |resp_msg: Message<E>|
            ...
            ==> (
                forall |other_resp: Message<E>| other_resp != resp_msg && #[trigger] ex.head().message_in_flight(other_resp)
        ...
        if Self::at_expected_reconcile_states(key, state)(s_prime) {
            ...
            match next_step {
                ...
                Step::KubernetesBusy(input) => {
                    if input == Some(s.pending_req_of(key)) {
                        let resp_msg = Message::form_matched_resp_msg(s.pending_req_of(key), Err(APIError::ServerTimeout));
                        assert(s_prime.message_in_flight(resp_msg));
                    } else {
                        if !s.message_in_flight(s.pending_req_of(key)) {
                            assert(s_prime.message_in_flight(resp));
