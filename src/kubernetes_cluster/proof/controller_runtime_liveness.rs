...
pub proof fn lemma_pre_leads_to_post_by_controller(
    spec: TempPred<Self>, input: (Option<Message<E>>, Option<ObjectRef>), next: ActionPred<Self>,
...
{
    use_tla_forall::<Self, (Option<Message<E>>, Option<ObjectRef>)>(
...
pub proof fn lemma_pre_leads_to_post_by_schedule_controller_reconcile_borrow_from_spec(
    spec: TempPred<Self>, input: ObjectRef, next: ActionPred<Self>,
    c: StatePred<Self>, pre: StatePred<Self>, post: StatePred<Self>
)
    requires
...
pub proof fn lemma_true_leads_to_reconcile_scheduled_by_assumption(
    ...
    ensures
        spec.entails(
            true_pred().leads_to(lift_state(|s: Self| s.reconcile_scheduled_for(cr_key)))
        ),
{
    ...
    let pre = |s: Self| true;
    let post = |s: Self| s.reconcile_scheduled_for(cr_key);
    ...
    requires
        ...
        spec.entails(
            lift_state(Self::at_expected_reconcile_states(cr.object_ref(), next_state))
                .leads_to(lift_state(|s: Self| !s.reconcile_state_contains(cr.object_ref())))
        ),
    ensures
        spec.entails(
        ...
        ),
{
    ...
    assert forall |msg: Message<E>| spec.entails(#[trigger] known_resp_in_flight(msg)
    ...
    leads_to_exists_intro::<Self, Message<E>>(spec, known_resp_in_flight, lift_state(post));
    assert_by(
        tla_exists(known_resp_in_flight) == lift_state(pre),
        {
            ...
            implies tla_exists(known_resp_in_flight).satisfied_by(ex) by {
                let s = ex.head();
                let msg = choose |resp_msg: Message<E>| {
                    #[trigger] s.message_in_flight(resp_msg)
        ...
        ),
{
    ...
    assert forall |req_msg: Message<E>| spec.entails(
