...
{
    assert forall |msg: Message<E>| #[trigger] s_prime.message_in_flight(msg) implies
        ...
        ),
{
    ...
    invariant(s_prime) by {
        assert forall |msg: Message<E>| msg.content.is_APIRequest() && #[trigger] s_prime.message_in_flight(msg) implies
...
{
    assert forall |msg: Message<E>| #[trigger] s_prime.message_in_flight(msg) implies
    (forall |other_msg: Message<E>| #[trigger] s_prime.message_in_flight(other_msg) && msg != other_msg
        ==> msg.content.get_rest_id() != other_msg.content.get_rest_id()) by {
        assert forall |other_msg: Message<E>| #[trigger] s_prime.message_in_flight(other_msg) && msg != other_msg implies
        msg.content.get_rest_id() != other_msg.content.get_rest_id() by {
...
proof fn newly_added_msg_have_different_id_from_existing_ones(
    s: Self, s_prime: Self, msg_1: Message<E>, msg_2: Message<E>
)
    requires
        Self::next()(s, s_prime),
        Self::every_in_flight_msg_has_lower_id_than_allocator()(s),
...
pub open spec fn req_in_flight_or_pending_at_controller(req_msg: Message<E>, s: Self) -> bool {
